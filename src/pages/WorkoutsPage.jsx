...
const formatTime = (totalSeconds) => {
  ...
  if (!numericSeconds || numericSeconds <= 0) {
    return '--:--'
  }
...
const getWorkoutExerciseCount = (workout) => {
  const initialSteps = Array.isArray(workout?.initialSteps)
    ? workout.initialSteps.length
    : 0

  const roundSteps = Array.isArray(workout?.rounds)
    ? workout.rounds.reduce((total, round) => {
        const steps = Array.isArray(round.steps)
          ? round.steps.length
          : 0
  ...
  const finalSteps = Array.isArray(workout?.finalSteps)
    ? workout.finalSteps.length
    : 0
    ...
    <main className="workouts-page">
      ...
      <section className="workouts-shell">
        <AppBrandHeader
          ...
          action={
            <div
              className="workouts-header-icon"
              aria-hidden="true"
            >
              <WorkoutIcon />
            </div>
        ...
        <section className="workouts-information-card">
          <div className="workouts-information-icon">
            <TrophyIcon />
          </div>
          ...
          <div>
            ...
            <p>
              Los workouts independientes no completan sesiones del
              programa, pero sí cuentan para tu historial y mejores
              tiempos.
            </p>
          </div>
        </section>
              ...
              >
                <div className="workout-card-image">
                  ...
                  {workout.requiresRunningSpace && (
                    <div className="workout-running-badge">
                      <RunningIcon />
                      <span>Requiere espacio para correr</span>
                    </div>
                  )}
                </div>
                ...
                <div className="workout-card-content">
                  <div className="workout-card-meta">
                    ...
                    <article>
                      <span>Bloques</span>
                      <strong>{totalExerciseSteps}</strong>
                    </article>
                  ...
                  <div className="workout-card-times">
                    ...
                    <div className="workout-time-item">
                      ...
                      <small>
                        {formatDate(statistics.lastCompletedAt)}
                      </small>
                    </div>
                  </div>
